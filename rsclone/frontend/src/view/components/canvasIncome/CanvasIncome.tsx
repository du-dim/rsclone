...
export const CanvasIncome = ({
  ...
  const [categoriesIncome, setCategoriesIncome] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    if (dataIncome.length) {
      ...
      for (let index = 0; index < amountIncome.length; index += 1) {
        dataEffect.push({
          color: colors[index],
          amount: amountIncome[index],
  ...
  return (
    <div className='chart'>
      <div className='chart_title'>Chart income</div>
      <canvas ref={canvasRef} width={width} height={height} />
      <div className='category'>
        {categoriesIncome.map((el, index) => (
          <div
